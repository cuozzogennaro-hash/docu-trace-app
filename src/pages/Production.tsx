  ...
  async function load() {
    ...
    if (isOperatorAdmin && operator) {
      ...
      const [rmRes, prRes] = await Promise.all([
        supabase.from("raw_materials").select("id, product_name, internal_lot, category, is_out_of_stock, created_at, department_id, expiry_date, born_in, raised_in, slaughtered_in, origin, ingredients").eq("is_out_of_stock", false).order("created_at", { ascending: false }),
