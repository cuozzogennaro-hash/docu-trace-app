...
Deno.serve(async (req) => {
  ...
  const { data: rows, error: updErr } = await supabase
    .from("scales_lotti_queue")
    .update({ status: "processed" })
    .eq("store_id", store.id)
    .eq("status", "pending")
    .select(
      "scale_slot_number, lot_code, born_in, raised_in, slaughtered_in, slaughterhouse_cee, department_code",
    );

  if (updErr) {
    console.error("lotti queue pull error", updErr);
    return json({ error: "internal_error" }, 500);
  }
