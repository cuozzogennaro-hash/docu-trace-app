...
-- Funzione che attiva un trial locale di 30 giorni senza carta.
-- Idempotente: se esiste già una subscription per (user, env), non fa nulla.
CREATE OR REPLACE FUNCTION public.start_local_trial(p_env text DEFAULT 'live')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.start_local_trial(text) TO authenticated;
